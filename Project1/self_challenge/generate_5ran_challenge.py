...
if __name__ == '__main__':
    ...
    equ = 0
    i = 0
    with open('ran_5_result.txt', 'r') as f:
        arr1 = f.readlines()[-1].split(': ')
        i = int(arr1[0])
        arr2 = arr1[1].split(', ')
        pha5_win = int(arr2[0].split('=')[1])
        ran_win = int(arr2[1].split('=')[1])
        equ = int(arr2[2].split('=')[1])
        f.close()
    f = open('ran_5_result.txt', 'a')
    try:
        while True:
            ...
            if result_1 + result_2 > 0:
                ...
                equ += 1
            f.write(f"{i}: pha5={pha5_win}, ran={ran_win}, equ={equ}\n")
            print(f"{i}: pha5={pha5_win}, ran={ran_win}, equ={equ}\n")
    except BaseException as e:
        f.write(f"{i}: ph5={pha5_win}, ran={ran_win}, equ={equ}\n")
        f.flush()
        f.close()
        raise e
