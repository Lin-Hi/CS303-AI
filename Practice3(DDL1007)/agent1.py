...
class ProblemSolvingAgent:
    ...
    def DFS(self, obstacles, start_pos, goal_pos):
        path, visited = [], [start_pos]
        last_node = {start_pos: None}
        q = [start_pos]
        node_hash = {start_pos: list(self.neighbours_of(obstacles, start_pos))}
        while len(q) > 0:
            if len(node_hash[q[-1]]) == 0:
                q.pop()
            else:
                now_pos = q[-1]
                next_pos = node_hash[q[-1]][-1][0]
                if next_pos in last_node.keys():
                    node_hash[q[-1]].pop()
                    continue
                last_node[next_pos] = now_pos
                visited.append(next_pos)
                if next_pos == goal_pos:
                    break
                node_hash[q[-1]].pop()
                q.append(next_pos)
                node_hash[next_pos] = list(self.neighbours_of(obstacles, next_pos))
        path.append(goal_pos)
        while path[-1] != start_pos:
            now_pos = path[-1]
            path.append(last_node[now_pos])
        path.reverse()
        return path, visited
    ...
    def BFS(self, obstacles, start_pos, goal_pos):
        ...
        while not q.empty():
            end_flag = False
            now_pos = q.get()
            neighbours = self.neighbours_of(obstacles, now_pos)
            for neighbour in neighbours:
                if neighbour[0] in last_node.keys():
                    continue
                q.put(neighbour[0])
                last_node[neighbour[0]] = now_pos
    ...
    def UCS(self, obstacles, start_pos, goal_pos):
        ...
        while not q.empty():
            end_flag = False
            now_pos_cost = q.get()
            if now_pos_cost[0] > min_cost[now_pos_cost[1]]:
                continue
            neighbours = self.neighbours_of(obstacles, now_pos_cost[1])
            for neighbour in neighbours:
                if neighbour[0] in min_cost.keys() and neighbour[1] + now_pos_cost[0] >= min_cost[neighbour[0]]:
                    continue
                q.put((neighbour[1] + now_pos_cost[0], neighbour[0]))
                min_cost[neighbour[0]] = neighbour[1] + now_pos_cost[0]
                last_node[neighbour[0]] = now_pos_cost[1]
                visited.append(neighbour[0])
                if neighbour[0] == goal_pos:
                    end_flag = True
                    break
