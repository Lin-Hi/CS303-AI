...
class ProblemSolvingAgent:
    ...
    def DFS(self, obstacles, start_pos, goal_pos):
        path, visited = [], []
        stack = [start_pos]
        now_last = {start_pos: None}
        node_neighbor = {start_pos: list(self.neighbours_of(obstacles, start_pos))}
        while len(stack) > 0:
            cur = stack[-1]
            while len(node_neighbor[cur]) > 0:
                if node_neighbor[cur][-1][0] in now_last.keys():
                    node_neighbor[cur].pop()
                else:
                    break
            if len(node_neighbor[cur]) == 0:
                stack.pop()
                continue
            neighbor = node_neighbor[cur]
            next_node = node_neighbor[cur][-1][0]
            visited.append(next_node)
            now_last[next_node] = cur
            if next_node == goal_pos:
                break
            neighbor.pop()
            stack.append(next_node)
            node_neighbor[next_node] = list(self.neighbours_of(obstacles, next_node))
        path.append(goal_pos)
        cur = goal_pos
        while path[-1] != start_pos:
            cur = now_last[cur]
            path.append(now_last[cur])
        path.reverse()
        return path, visited
