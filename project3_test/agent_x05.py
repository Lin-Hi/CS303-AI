...
class Agent:
    def __init__(self) -> None:
        ...
        self.net = Net()
        PATH = path.join(path.dirname(__file__), '../project3/classifier_para.pth')
    ...
    def get_action(self,
        ...
        while time() - start_time < 0.20:
            random_ans = torch.rand((N_CTPS - 2, 2)) * torch.tensor([5, 6]) + torch.tensor([0, -3.])
            score_real = evaluate(compute_traj(random_ans), target_pos, class_scores[target_cls], RADIUS)
...
def evaluate_modified(
    ...
    hit = (d < radius)
    d[hit] = 1
    d[~hit] = torch.pow(RADIUS / d[~hit], 0.5)
