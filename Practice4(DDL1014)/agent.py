    ...
    def Astar(self, obstacles, start_pos, goal_pos):
        ...
        while len(heap) != 0:
            ...
            if cur == goal_pos:
                ...
                return path, visited
            for neighbor, cost in list(self.neighbours_of(obstacles, cur)):
                dist = self.euclidean_distance(neighbor, goal_pos)
                if neighbor not in open_g.keys() and neighbor not in close_g.keys():
                    heapq.heappush(heap, (cur_g + cost + dist, neighbor))
                    open_g[neighbor] = cur_g + cost
                    parents[neighbor] = cur
                elif neighbor in open_g.keys():
                    pre_g = open_g[neighbor]
                    if cur_g + cost < pre_g:
                        heap.remove((pre_g + dist, neighbor))
                        heapq.heappush(heap, (cur_g + cost + dist, neighbor))
                        open_g[neighbor] = cur_g + cost
                        parents[neighbor] = cur
                # else:
                #     pre_g = close_g[neighbor]
                #     if cur_g + cost < pre_g:
                #         del close_g[neighbor]
                #         open_g[neighbor] = cur_g + cost
                #         heapq.heappush(heap, (cur_g + cost + dist, neighbor))
                #         parents[neighbor] = cur
            close_g[cur] = cur_g
